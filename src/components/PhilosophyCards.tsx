  ...
  return <motion.div ref={ref} initial={{
    ...
    delay: 0.3
      ...
      <div className="relative h-40 flex items-center justify-center overflow-hidden">
        {/* Infinity Loop Visualization - More compact layout */}
        <div className="relative flex items-center gap-4">
          {/* Agent Node */}
          <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-cyan-500/20 to-cyan-500/5 border border-cyan-500/30 flex flex-col items-center justify-center z-10">
            <span className="text-lg">🤖</span>
            <span className="text-[9px] text-cyan-400 mt-0.5">Agent</span>
          </div>

          {/* Infinity Loop Connection */}
          <div className="relative w-24 h-16">
            {/* Infinity symbol path (visual guide) */}
            <svg className="absolute inset-0 w-full h-full" viewBox="0 0 96 64">
              <path d="M 24 32 C 24 16, 48 16, 48 32 C 48 48, 72 48, 72 32 C 72 16, 48 16, 48 32 C 48 48, 24 48, 24 32" fill="none" stroke="hsl(var(--border))" strokeWidth="1.5" strokeDasharray="4 4" opacity="0.4" />
            </svg>
            ...
            delay: i * 0.5,
            ease: "easeInOut"
          }} className="absolute top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-cyan-400 shadow-[0_0_6px_hsl(186_100%_50%/0.8)]" />)}
            ...
            delay: i * 0.5,
            ease: "easeInOut"
          }} className="absolute top-1/2 -translate-y-1/2 w-1.5 h-1.5 rounded-full bg-purple-400 shadow-[0_0_6px_hsl(270_80%_60%/0.8)]" />)}

            {/* Center infinity icon */}
            <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 text-muted-foreground/40 text-sm font-light">
              ∞
            </div>
          </div>

          {/* Viking Node */}
          <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-purple-500/20 to-purple-500/5 border border-purple-500/30 flex flex-col items-center justify-center z-10">
            <img src={vikingLogo} alt="Viking" className="w-8 h-8 object-contain" />
          </div>
        </div>

        {/* Toast Notifications / Terminal Logs - Adjacent to visualization */}
        <div className="absolute right-2 top-1 bottom-1 w-28 flex flex-col justify-center gap-1 overflow-hidden">
          ...
          opacity: 1,
          x: 0
        }} className="bg-slate-800/80 border border-purple-500/30 rounded px-1.5 py-0.5 text-[8px] text-purple-300/90 font-mono truncate">
              <span className="text-purple-400">›</span> {log}
            </motion.div>)}
        </div>

        {/* Flow labels */}
        <div className="absolute bottom-0.5 left-1/2 -translate-x-1/2 flex gap-3 text-[7px]">
          <span className="text-cyan-400/60">← Context Supply</span>
          <span className="text-purple-400/60">Feedback →</span>
        </div>
      </div>
    </motion.div>;
};
